...
void MultiSenseSL::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  // Get imu link
  this->imuLink = this->parentModel->GetLink(this->imuLinkName);
  if (!this->imuLink)
  ...
  this->spindleLink = this->parentModel->GetLink("hokuyo_link");
  if (!this->spindleLink)
  {
  ...
  this->spindleJoint = this->parentModel->GetJoint("hokuyo_joint");
